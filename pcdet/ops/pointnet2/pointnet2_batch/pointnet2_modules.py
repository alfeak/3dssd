...
class PointNorm(nn.Module):
    ...
    def forward(self, x):
        points,anchor = x
        # B,N,K,D = points.shape
        # anchor = points[:,:,0,:].unsqueeze(-2)
        ...
        std = torch.std((points-anchor),dim=(1,2,3),keepdim=True) #[b,n]
        points = self.alpha * (points-anchor)/(std+1e-4) + self.beta + anchor
        return points
...
class PointnetSAModuleMSG_SSD(_PointnetSAModuleBase):
    ...
    def __init__(self, *, npoint: int, radii: List[float], nsamples: List[int], mlps: List[List[int]], bn: bool = True,
        ...
        for i in range(len(radii)):
            ...
            shared_mlps = []
            # shared_nroms = []
            # shared_nroms.extend([PointNorm(mlp_spec[0])])
            # self.norms.append(nn.Sequential(*shared_nroms))
            for k in range(len(mlp_spec) - 1):
                shared_mlps.extend([
                    nn.Conv2d(mlp_spec[k], mlp_spec[k + 1], kernel_size=1, bias=False),
                    nn.BatchNorm2d(mlp_spec[k + 1]),
    ...
    def forward(self, xyz: torch.Tensor, features: torch.Tensor = None, new_xyz=None, ctr_xyz=None) -> (torch.Tensor, torch.Tensor):
        ...
        if len(self.groupers) > 0:
            for i in range(len(self.groupers)):
                new_features = self.groupers[i](xyz, new_xyz, features)  # (B, C, npoint, nsample)
                # if ctr_xyz is None:
                #     sampled_features = pointnet2_utils.gather_operation(features, fps_idxes).contiguous()
                #     if self.use_xyz:
                #         sampled_features = torch.cat([new_xyz.transpose(1, 2).contiguous(), sampled_features], dim=1)
                #     new_features = self.norms[i]((new_features, sampled_features))
                new_features = self.mlps[i](new_features)  # (B, mlp[-1], npoint, nsample)
                if self.pool_method == 'max_pool':
                    new_features = F.max_pool2d(
                        new_features, kernel_size=[1, new_features.size(3)]
