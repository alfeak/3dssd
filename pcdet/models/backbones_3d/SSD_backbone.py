    ...
    def forward(self, batch_dict):
        ...
        for i in range(len(self.SA_modules)):
            ...
            if self.layer_types[i] == 'SA_Layer':
                ctr_xyz = None
                # print("input: ", xyz_input.shape, feature_input.shape)
                if self.ctr_indexes[i] != -1:
                    ctr_xyz = encoder_xyz[self.ctr_indexes[i]]
                    # print("last input: ",xyz_input.shape,feature_input.shape,ctr_xyz.shape)
                li_xyz, li_features = self.SA_modules[i](xyz_input, feature_input, ctr_xyz=ctr_xyz)
                # print("output: ",li_xyz.shape,li_features.shape)
            elif self.layer_types[i] == 'Vote_Layer':
                # print("vote input: ", xyz_input.shape, feature_input.shape)
                li_xyz, li_features, ctr_offsets = self.SA_modules[i](xyz_input, feature_input)
                # print("vote output: ",li_xyz.shape,li_features.shape,ctr_offsets.shape)
                centers = li_xyz
